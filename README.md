...
v1.3 采用参考 Codex 的中性配色与中文字体层次，精简设置栏和监控操作，并修复签到结果误报成功、停止监控后采集线程未退出的问题。

## 界面截图

以下为 v1.3 的真实桌面界面，使用演示账号和模拟转写内容。截图直接从程序窗口导出为无损 PNG，不含鼠标指针和操作光晕；点击图片可查看原图。
...
- 双栏桌面界面：简洁侧栏、转写面板和活动记录；主按钮随运行状态切换，支持深浅色切换并记住选择，优先使用已安装的中等字重中文无衬线字体。
...
5. 启动后，底部主按钮会变为「停止监控」，结束时点击即可。监控期间修改识别或通知设置，需要停止后重新启动才能应用。
...
在 Windows 开发环境中运行以下命令，会使用临时配置和模拟数据重新生成深浅色截图，不连接课堂、登录服务或通知接口：

```powershell
.\.venv\Scripts\python.exe tools/capture_screenshots.py
```

默认按 1.5 倍尺寸实际渲染并输出 PNG，不对旧图进行放大。屏幕空间不足时可加 `--scale 1`；加 `--preview` 可交互检查演示界面。截图只包含软件内容区域，账号、转写和签到码均为演示数据。
